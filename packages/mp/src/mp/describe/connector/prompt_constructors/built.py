...
class BuiltPromptConstructor(ConnectorPromptConstructor):
    ...
    async def construct(self) -> str:
        ...
        return template.safe_substitute({
            ...
            "python_file_content": await self._get_built_connector_content(),
    ...
    async def _get_built_connector_def_content(self) -> str:
        connector_def: anyio.Path = (
            self.out_path
            / constants.OUT_CONNECTORS_META_DIR
            / f"{self.resource_file_name}{constants.CONNECTORS_META_SUFFIX}"
        )
        if await connector_def.exists():
            return await connector_def.read_text(encoding="utf-8")

        return DEFAULT_FILE_CONTENT

    async def _get_built_connector_content(self) -> str:
        connector_script: anyio.Path = (
            self.out_path / constants.OUT_CONNECTOR_SCRIPTS_DIR / f"{self.resource_file_name}.py"
        )
