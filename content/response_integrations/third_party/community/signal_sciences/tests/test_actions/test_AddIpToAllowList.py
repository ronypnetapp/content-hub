...
class TestAddIpToAllowList:
    ...
    def test_add_ip_to_allow_list_already_exists(
        ...
        signal_sciences.add_ip_to_allowlist(corp_name, site_name, "1.2.3.4", "Existing Note")

        # Act
        AddIpToAllowList.main()

        # Assert
        # Should NOT call PUT if it already exists (the action script checks first)
    ...
    def test_add_ip_to_allow_list_with_entities(
        self,
        script_session: SignalSciencesSession,
        action_output: MockActionOutput,
        signal_sciences: SignalSciences,
