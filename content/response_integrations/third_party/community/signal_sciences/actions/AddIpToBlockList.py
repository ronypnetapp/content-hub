...
SCRIPT_NAME = "Add IP to Block List"
...
class AddIpToBlockListAction(SignalSciencesAction):
    ...
    def _perform_action(self, _=None) -> None:
        ...
        for ip_address in target_ips:
            ...
            try:
                raw_data = self.api_client.add_ip_to_blocklist(
    ...
    def _finalize_action_on_success(self) -> None:
        ...
        if self.successful_ips:
            self.output_message = (
                f"Successfully added the following IPs to the Block List for "
                f"site {self.site_name} in Signal Sciences:\n{', '.join(self.successful_ips)}\n"
            )

        if self.failed_ips:
            error_msg = (
                f"Failed to add the following IPs to the Block List for site "
