...
SCRIPT_NAME = "Remove IP from Allow List"


class RemoveIpFromAllowListAction(SignalSciencesAction):
    def __init__(self):
        ...
        for ip_address in target_ips:
            ...
            try:
                ...
                for item in matching_items:
                    self.api_client.remove_ip_from_allowlist(
    ...
    def _finalize_action_on_success(self) -> None:
        ...
        if self.successful_ips:
            self.output_message = (
                f"Successfully removed the following IPs from the Allow List for "
                f"site {self.site_name} in Signal Sciences:\n{', '.join(self.successful_ips)}\n"
            )

        if self.failed_ips:
            error_msg = (
                f"Failed to remove the following IPs from the Allow List for "
...
def main():
    RemoveIpFromAllowListAction().run()
