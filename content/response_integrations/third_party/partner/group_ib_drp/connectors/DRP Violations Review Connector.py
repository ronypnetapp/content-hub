...
@output_handler
def main() -> None:
    """Run the DRP Violations Review connector ingestion cycle.

    Targets violations whose ``approveState`` is ``under_review`` so analysts
    can triage them in SOAR. Reads the connector's script parameters, pulls
    a single portion of review-pending violations from the Group-IB DRP API
    via ``gather_events``, builds one ``AlertInfo`` per event with a
    ...
    siemplify.script_name = Config.GC_REVIEW_CONNECTOR_SCRIPT_NAME
...
def gather_events(siemplify, start_date):
    """Pull a single portion of under-review violations from the DRP API.
    ...
    ``state`` is a dict carrying the cursor info. Server-side filter
    ``approve_states=[3]`` restricts the feed to violations awaiting
    review. Polling one portion per scheduled run is intentional: it
    bounds memory and runtime, and the persisted ``sequpdate`` cursor
    lets the next run pick up where this one left off.
    ``save_timestamp`` is intentionally NOT called here — the cursor
    must only be persisted after ``return_package`` succeeds in
    ``main()`` so a crash leaves the cursor unchanged and SOAR's
