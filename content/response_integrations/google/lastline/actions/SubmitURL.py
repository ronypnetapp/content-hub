...
def query_operation_status(
    ...
    if submission_task.data.completed:
        ...
        status = EXECUTION_STATE_INPROGRESS
        result_value = json.dumps({"task_uuid": task_uuid, "url": url})
        output_message = f"Waiting for the analysis results for the url: {url}"
...
def main(is_first_run):
    ...
    siemplify.script_name = f"{INTEGRATION_NAME} - {SUBMIT_URL}"
    ...
    url = extract_action_param(
        siemplify, param_name="URL For Analysis", is_mandatory=False, print_value=True
    )
    ...
    try:
        ...
        if is_first_run:
            ...
            url = json.loads(siemplify.extract_action_param("additional_data")).get(
                "url"
            )
